...
module.exports.status = function () {
  if (!(f.exists(confFile) && f.exists(confHook))) {
    log(chalk.bold('\n\t❗ IdeaBlock Commit is currently ') + chalk.bold.red('UNINITIALIZED') + chalk.bold(' in this directory.\nPlease run the "init" function in this directory to initialize IdeaBlock Commit.\n'))
  } else if (!f.isRepo()) {
    ...
    log('\n\tIdeaBlock Commit is currently ' + chalk.bold.green('INITIALIZED ') + 'and ' + chalk.bold.red('OFF') + ' in this repository.\n')
  }
}
