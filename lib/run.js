  ...
  fs.pathExists(path.join(process.cwd(), '.ideablock', 'ideablock.json'))
    .then((exists) => {
      if (!exists) {
        ...
        console.log('Please run "ideablock-commit init" in the current directory before invoking the run function.')
      } else {
        fs.readJSON(path.join(process.cwd(), '.ideablock', 'ideablock.json'))
          .then((obj) => {
            if (obj.on === true) return true
            else if (obj.on === false) return false
...
module.exports.isOff = () => {
  if (this.isOn() === undefined) return undefined
