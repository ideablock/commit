...
module.exports.on = function () {
  if (!f.isRepo()) {
    ...
    log('\nIdeaBlock is currently set to ' + chalk.bold.red('ON') + ' in this repository.\n')
  } else if (f.isOff()) {
    fs.writeJSON(confFile, { on: true })
      .then(() => fs.copy(confHook, gitHook))
      .then(() => fs.chmod('u+x', gitHook))
      .then(() => log('\nIdeaBlock Commit has been set to ' + chalk.bold.red('ON') + ' in this repository.\n'))
