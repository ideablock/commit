...
module.exports.on = function () {
  if (!f.isRepo()) {
    ...
    log('\n\t✅  ' + chalk.bold.black('IdeaBlock Commit is currently set to ') + chalk.bold.green('ON') + chalk.bold.black(' in this repository.\n'))
  } else if (f.isOff()) {
    fs.writeJsonSync(confFile, { on: true })
    fs.copySync(confHook, gitHook)
    fs.chmod(gitHook, 0o744, function (err) {
      if (err) console.log(err)
      log('\n✅  ' + chalk.bold.black('IdeaBlock Commit has been set to ') + chalk.bold.green('ON') + chalk.bold.black(' in this repository.\n'))
      process.exit(0)
    })
  }
}
