...
const createHook = function (err, results) {
  ...
  fs.copy(path.join('.ideablock', 'post-commit'), path.join(process.cwd(), '.git', 'hooks', 'post-commit'))
...
module.exports.init = function () {
  if (f.isRepo() && f.exists(path.join(process.cwd(), '.ideablock'))) {
    console.log('\nRe-initializing IdeaBlock Commit in current directory.\n')
    async.series([addIgnoreLine, createDotIdeaBlock], createHook)
  } else if ((f.isRepo() && !f.exists(path.join(process.cwd(), '.ideablock')))) {
    console.log('\nInitializing IdeaBlock Commit in current directory.\n')
    async.series([addIgnoreLine, createDotIdeaBlock], createHook)
  } else if (!f.isRepo()) {
    console.log('\nThe present directory is not a git repository.')
